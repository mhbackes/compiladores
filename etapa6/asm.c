...
void asmAttrArr(FILE *file, TAC *tac) {
    ...
    switch(dst->datatype) {
        case DTYPE_CHAR:
            ...
            fprintf(file, "\tmov\t%%al, %s+%d(%%rip)\n", dst->name, offset);
            break;
        case DTYPE_INT:
            asmConvertToInt(file, val, 'a', 0);
            fprintf(file, "\tmovl\t%%eax, %s+%d(%%rip)\n", dst->name, offset);
            break;
        case DTYPE_REAL:
            asmConvertToReal(file, val, '0', 0);
            fprintf(file, "\tmovss\t%%xmm0, %s+%d(%%rip)\n", dst->name, offset);
            break;
        case DTYPE_BOOL:
            fprintf(file, "\tmov\t%s(%%rip), %%al\n", val->name);
            fprintf(file, "\tmov\t%%al, %s+%d(%%rip)\n", dst->name, offset);
...
void asmGE(FILE *file, TAC *tac) {
    fprintf(file, "/* TAC_GE %s = %s >= %s */\n", tac->res->text, 
            tac->op1->text, tac->op2->text);
    asmBooleanComparison(file, tac, "setge");
...
void asmLESS(FILE *file, TAC *tac) {
    fprintf(file, "/* TAC_LE %s = %s < %s */\n", tac->res->text, 
            tac->op1->text, tac->op2->text);
    asmBooleanComparison(file, tac, "setl");
...
void asmGREATER(FILE *file, TAC *tac) {
    fprintf(file, "/* TAC_LE %s = %s > %s */\n", tac->res->text, 
            tac->op1->text, tac->op2->text);
    asmBooleanComparison(file, tac, "setg");
}
